...
- **CRUD référentiel services** : GET/POST/PATCH `/infrastructure/departments` (infrastructure.manage) — POST auto-génère le code (4 lettres sans accents + suffixe numérique) si absent, unicité nom insensible à la casse → 409 ; **jamais de DELETE** : désactivation (is_active=false), l'historique est préservé ; **rename → propagation service_name aux rooms + occupancy_beds dans la même transaction** (client pool.connect, PAS pool.query BEGIN). UI : onglet Services dans InfrastructureManager (ServicesTab.tsx, compteurs d'usage par service).
- **Frontend : hook `useServices()` (src/hooks/useServices.ts) = source UNIQUE des listes de services** — plus aucune liste hardcodée (AdmissionFilters, ConsultationFilters, TabDecision/FormHospitalisation, ConsumptionsPage qui ajoute « Autre »). ConsultationFilters lit aussi les médecins réels via /directory/doctors (valeur filtre = fullName).
...
- Permission `infrastructure.manage` (module 'admissions' en DB) grantée à super_admin/system_administrator/administrator ; lecture tree/bed-cards = admissions.view ; **GET /infrastructure/services = requireAuth seul** (référentiel transverse lisible par tout le personnel authentifié — les utilisateurs stock/consultations n'ont pas forcément admissions.view). **Nouvelle permission DB ⇒ re-login requis** pour les rôles non-super_admin (perms embarquées dans le JWT).
