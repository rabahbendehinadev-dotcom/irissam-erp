...
import { getWorkOrders, createWorkOrder, startWorkOrder, closeWorkOrder,
         getEquipment } from "@/services/api/biomedical";
...
export default function WorkOrdersPage() {
  ...
  const [typeF, setTypeF] = createSignal("");
  const [showCreate, setShowCreate] = createSignal(false);
  const [form, setForm] = createSignal<Record<string,any>>({ tasks: [] });
  const [taskInput, setTaskInput] = createSignal("");

  const [equipSearch, setEquipSearch] = createSignal("");
  const [equipList] = createResource(() => equipSearch(), q => getEquipment({ q, limit: 20 }));

  const [data, { refetch }] = createResource(
    () => ({ page: page(), status: statusF(), order_type: typeF() }),
    p => getWorkOrders({ ...p, limit: 20 })
  );
  ...
  return (
    <div class="space-y-4">
      <div class="flex flex-col sm:flex-row gap-2">
        ...
        <select class="border border-gray-300 rounded-lg px-3 py-2 text-sm"
          value={typeF()} onChange={e => setTypeF(e.currentTarget.value)}>
      ...
      <div class="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
          ...
          <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
              ...
              <Show when={!data.loading && !data()?.data?.length}>
                <tr><td colspan="8" class="text-center py-10 text-gray-400">Aucun ordre de travail</td></tr>
              </Show>
              <For each={data()?.data}>
                {(wo: any) => (
                  <tr class="hover:bg-gray-50">
                    <td class="px-4 py-3 font-mono text-xs text-gray-600">{wo.order_number}</td>
                    <td class="px-4 py-3">
                      <div class="font-medium text-gray-900 text-xs">{wo.equipment_name}</div>
                      <div class="text-xs text-gray-400">{wo.internal_code}</div>
                    </td>
                    <td class="px-4 py-3">
                      <span class={`px-2 py-0.5 rounded-full text-xs font-medium ${TYPE_BADGE[wo.order_type]??""}`}>{wo.order_type}</span>
                    </td>
                    <td class="px-4 py-3 text-xs capitalize text-gray-600">{wo.priority}</td>
                    <td class="px-4 py-3">
                      <span class={`px-2 py-0.5 rounded-full text-xs font-medium ${STATUS_BADGE[wo.status]??""}`}>
                    ...
                    <td class="px-4 py-3 flex gap-1">
                      {wo.status === "ouvert" &&
                        <button onClick={() => handleAction(wo,"start")}
                          class="text-xs text-amber-600 hover:text-amber-800 font-medium">Démarrer</button>}
                      {["ouvert","en_cours","en_attente_pieces"].includes(wo.status) &&
                        <button onClick={() => handleAction(wo,"close")}
                          class="text-xs text-emerald-600 hover:text-emerald-800 font-medium ml-2">Clôturer</button>}
                    </td>
                  </tr>
                )}
              </For>
        ...
        <div class="flex items-center justify-between px-4 py-3 border-t border-gray-100">
          <p class="text-xs text-gray-500">Total: {data()?.total ?? 0}</p>
          <div class="flex gap-2">
            <button disabled={page()===1} onClick={() => setPage(p=>p-1)}
              class="px-3 py-1 text-xs border border-gray-300 rounded-lg disabled:opacity-40">Préc.</button>
            <span class="px-3 py-1 text-xs">{page()}</span>
            <button disabled={(data()?.total??0)<=page()*20} onClick={() => setPage(p=>p+1)}
              class="px-3 py-1 text-xs border border-gray-300 rounded-lg disabled:opacity-40">Suiv.</button>
          </div>
        </div>
      </div>
        ...
        <div class="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
          <div class="bg-white rounded-2xl shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-6 border-b">
              <h2 class="text-lg font-semibold">Nouvel ordre de travail</h2>
              <button onClick={() => setShowCreate(false)} class="text-gray-400 hover:text-gray-600">✕</button>
            </div>
            <form onSubmit={handleCreate} class="p-6 space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    value={form().order_type ?? "corrective"} onChange={e => setForm(p=>({...p,order_type:e.currentTarget.value}))}>
                    {["preventive","corrective","urgente","inspection","calibration","installation"].map(t =>
                      <option value={t}>{t}</option>)}
                  </select>
                </div>
                <div>
                  <label class="text-xs font-medium text-gray-600">Priorité</label>
                  <select class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    value={form().priority ?? "normale"} onChange={e => setForm(p=>({...p,priority:e.currentTarget.value}))}>
                    {["faible","normale","haute","critique"].map(c => <option value={c}>{c}</option>)}
                  </select>
                </div>
              </div>
              <div>
                <label class="text-xs font-medium text-gray-600">Rechercher équipement *</label>
                <input class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                  placeholder="Nom ou code…" value={equipSearch()} onInput={e => setEquipSearch(e.currentTarget.value)} />
                <Show when={equipList()?.data?.length}>
                  <div class="mt-1 border border-gray-200 rounded-lg max-h-36 overflow-y-auto">
                    <For each={equipList()?.data}>
                      {(eq: any) => (
                        <button type="button" class="w-full text-left px-3 py-2 hover:bg-indigo-50 text-sm border-b last:border-0"
                          ...
                          <span class="text-gray-400 ml-2 text-xs">{eq.internal_code}</span>
                        </button>
                      )}
                    </For>
                  </div>
              ...
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="date" class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    value={form().scheduled_date ?? ""} onInput={f("scheduled_date")} />
                </div>
                <div>
                  <label class="text-xs font-medium text-gray-600">Heures estimées</label>
                  <input type="number" min="0" step="0.5" class="mt-1 w-full border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    value={form().estimated_hours ?? ""} onInput={f("estimated_hours")} />
                </div>
              </div>
              <div>
              ...
              <div>
                <label class="text-xs font-medium text-gray-600">Tâches</label>
                <div class="flex gap-2 mt-1">
                  <input class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm"
                    placeholder="Ajouter une tâche…" value={taskInput()} onInput={e => setTaskInput(e.currentTarget.value)}
                    onKeyDown={e => e.key==="Enter" && (e.preventDefault(), addTask())} />
                  <button type="button" onClick={addTask}
                    class="px-3 py-2 bg-gray-100 rounded-lg text-sm hover:bg-gray-200">+</button>
                </div>
                <For each={form().tasks}>
                  {(t: any, i) => (
                    <div class="flex items-center gap-2 mt-1">
                      <span class="text-xs text-gray-500 w-5">{i()+1}.</span>
                      <span class="text-sm flex-1">{t.task_name}</span>
              ...
              <div class="flex justify-end gap-3 pt-2">
                <button type="button" onClick={() => setShowCreate(false)}
                  class="px-4 py-2 text-sm border border-gray-300 rounded-lg">Annuler</button>
                <button type="submit" class="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Créer OT</button>
              </div>
            </form>
          </div>
        </div>
