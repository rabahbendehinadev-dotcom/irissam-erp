import { Shield, CheckCircle, XCircle, Clock, FileText, History } from 'lucide-react';
...
const MOCK_HISTORY: InsuranceHistoryEntry[] = [
  { id: 'ih-1', type: 'CNAS', organization: 'CNAS Alger Centre',    startDate: '2024-01-01', endDate: '2024-12-31', status: 'expired' },
  { id: 'ih-2', type: 'CNAS', organization: 'CNAS Alger Centre',    startDate: '2025-01-01', endDate: '2025-12-31', status: 'active'  },
];
...
function StatusBadge({ status }: { status: InsuranceHistoryEntry['status'] }) {
  const map = {
...
export function PatientInsuranceDetail({ patient }: Props) {
  ...
  // Deterministic mock coverage based on insurance type
    ...
    <div className="max-w-2xl space-y-4">
              ...
              {expired ? (
                <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                  ...
                  <p className="text-sm font-medium text-red-700">
                    Assurance expirée le {formatDate(ins.validUntil ?? '')}
                  </p>
                </div>
              ) : (
                <div className="flex items-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <p className="text-sm font-medium text-green-700">
                    Assurance valide{ins.validUntil ? ` jusqu'au ${formatDate(ins.validUntil)}` : ''}
                  </p>
                </div>
              )}
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-3 border-b border-gray-100 bg-gray-50">
          ...
          <button
            onClick={() => alert('Upload disponible avec le backend')}
